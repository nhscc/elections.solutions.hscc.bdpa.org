...
module.exports = {
    populateEnv() {
        ...
        if(!Array.isArray(expectedEnvVariables))
            throw new Error('Property expectedEnvVariables in package.json must be an array');

        const throwEnvError = variable => {
            throw new Error(`Environment variable ${variable} is improperly defined. Did you copy dist.env -> .env ?`);
        };
